  ...
  def wrapper(*args):
    ...
    if len(meshes_and_fns) > 1:
      ...
      for scratch_shape in flat_scratch_shapes:
        ...
        if (not isinstance(scratch_shape.memory_space, pallas_core.CoreMemorySpace)
    ...
    for mesh, _ in meshes_and_fns:
      meshes.append(mesh)
    for aval in [*flat_avals, *flat_out_avals, *flat_scratch_shapes]:
      if isinstance(aval, jax_core.ShapedArray):
        if isinstance(aval.memory_space, pallas_core.CoreMemorySpace):
          if aval.memory_space.mesh not in [*meshes, *external_meshes]:
