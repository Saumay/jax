...
def _get_prng_key_info(
  ...
  """
  return {
      idx: spec.dtype._impl  # pylint: disable=protected-access
...
def _convert_specs_to_physical(
    specs: tuple[api.ShapeDtypeStruct, ...],
    prng_info: dict[int, Any],
) -> tuple[api.ShapeDtypeStruct, ...]:
  """Converts PRNG key specs to physical specs, leaving others unchanged."""
  if not prng_info:
    return specs
  return tuple(
      _prng_key_spec_to_physical_spec(spec) if idx in prng_info else spec
      for idx, spec in enumerate(specs)
  )
...
def _wrap_prng_keys(
  ...
  result = list(results)
  if not prng_info:
    return result  # No PRNG keys to convert.
...
def _make_prng_wrapped_fun(
    fun: Callable[..., Any],
    prng_in_info: dict[int, Any],
    prng_out_info: dict[int, Any],
) -> Callable[..., Any]:
  ...
  try:
    ...
    if "PjRtCompiler requires an HloProgram" in str(e):
      ...
      def fallback_call(*args, **kwargs):
        ...
        results = deserialized_fun(*args, **kwargs)
